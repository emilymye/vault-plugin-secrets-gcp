...
func (b *backend) pathStaticAccountExistenceCheck(ctx context.Context, req *logical.Request, d *framework.FieldData) (bool, error) {
	...
	acct, err := getStaticAccount(nameRaw.(string), ctx, req.Storage)
	if err != nil {
		return false, err
	}
...
func (b *backend) pathStaticAccountRead(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	data := map[string]interface{}{
		...
		"bindings":                acct.Bindings.asOutput(),
	}
...
func (b *backend) pathStaticAccountDelete(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	acct, err := getStaticAccount(name, ctx, req.Storage)
	if err != nil {
...
func (b *backend) pathStaticAccountCreate(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	saveWarns, err := b.updateStaticAccountAndSave(ctx, req, nil, input)
	if err != nil {
		return logical.ErrorResponse(err.Error()), nil
	}
	warnings = append(warnings, saveWarns...)
	if len(warnings) > 0 {
		return &logical.Response{Warnings: warnings}, nil
	}
	return nil, nil
...
func (b *backend) pathStaticAccountUpdate(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	updateWarns, err := b.updateStaticAccountAndSave(ctx, req, acct, updateInput)
	if err != nil {
...
func (b *backend) updateStaticAccountAndSave(ctx context.Context, req *logical.Request, a *StaticAccount, input *inputParams) (warnings []string, err error) {
	...
	if err != nil {
		if isGoogleAccountNotFoundErr(err) {
			return nil, fmt.Errorf("unable to create static account - service account %q should exist", input.serviceAccountEmail)
		}
		return nil, errwrap.Wrapf(fmt.Sprintf("unable to create static account, could not confirm service account %q exists: {{err}}", input.serviceAccountEmail), err)
	}
	...
	var oldResources *gcpAccountResources
	var oldWalIds []string
	if a != nil {
		oldResources = a.boundResources()
		oldWalIds, err = b.addWalsForStaticAccountResources(ctx, req, input.name, oldResources)
		if err != nil {
			b.tryDeleteWALs(ctx, req.Storage, oldWalIds...)
			return nil, err
		}
	}

	newWalIds, err := b.addWalsForStaticAccountResources(ctx, req, input.name, oldResources)
	if err != nil {
		b.tryDeleteWALs(ctx, req.Storage, oldWalIds...)
	...
	// Edit static account with newly created resources and save to storage.
	a.Bindings = newResources.bindings
	a.TokenGen = newResources.tokenGen
	...
	// Try cleaning up old resources.
	return b.tryDeleteStaticAccountResources(ctx, req, oldResources, oldWalIds), nil
}

func getStaticAccount(name string, ctx context.Context, s logical.Storage) (*StaticAccount, error) {
